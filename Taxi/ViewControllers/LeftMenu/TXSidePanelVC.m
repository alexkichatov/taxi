...
@interface TXSidePanelVC() {
    CGRect _centerPanelRestingFrame;		
    CGPoint _locationBeforePan;
}

@property (nonatomic, readwrite) TXSidePanelState state;
...
- (void)_baseInit {
    self.style = TXSidePanelSingleActive;
...
- (void)viewDidLoad {
    ...
    self.state = TXSidePanelCenterVisible;
...
- (void)willAnimateRotationToInterfaceOrientation:(__unused UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    if (self.centerPanelHidden) {
        ...
        frame.origin.x = self.state == TXSidePanelLeftVisible ? self.centerPanelContainer.frame.size.width : -self.centerPanelContainer.frame.size.width;
...
- (void)setState:(TXSidePanelState)state {
    if (state != _state) {
        _state = state;
        switch (_state) {
            case TXSidePanelCenterVisible: {
...
- (void)setStyle:(TXSidePanelStyle)style {
    if (style != _style) {
        _style = style;
...
- (void)_layoutSideContainers:(BOOL)animate duration:(NSTimeInterval)duration {
    ...
    if (self.style == TXSidePanelMultipleActive) {
...
- (void)setCenterPanel:(UIViewController *)centerPanel {
    ...
    if (centerPanel != _centerPanel) {
        ...
        if (self.state == TXSidePanelCenterVisible) {
            self.visiblePanel = _centerPanel;
        }
    }
    if (self.isViewLoaded && self.state == TXSidePanelCenterVisible) {
        ...
        TXSidePanelState previousState = self.state;
        self.state = TXSidePanelCenterVisible;
        [UIView animateWithDuration:0.2f animations:^{
            if (self.bounceOnCenterPanelChange) {
                // first move the centerPanel offscreen
                CGFloat x = (previousState == TXSidePanelLeftVisible) ? self.view.bounds.size.width : -self.view.bounds.size.width;
...
- (void)_swapCenter:(UIViewController *)previous previousState:(TXSidePanelState)previousState with:(UIViewController *)next {
    if (previous != next) {
...
- (void)setLeftPanel:(UIViewController *)leftPanel {
    if (leftPanel != _leftPanel) {
        ...
        if (self.state == TXSidePanelLeftVisible) {
...
- (void)setRightPanel:(UIViewController *)rightPanel {
    if (rightPanel != _rightPanel) {
        ...
        if (self.state == TXSidePanelRightVisible) {
...
- (void)_handlePan:(UIGestureRecognizer *)sender {
    ...
    if ([sender isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (self.style == TXSidePanelMultipleActive || self.pushesSidePanels) {
...
- (CGFloat)_correctMovement:(CGFloat)movement {
    ...
    } else if (self.state == TXSidePanelRightVisible && !self.allowRightOverpan) {
    ...
    } else if (self.state == TXSidePanelLeftVisible  && !self.allowLeftOverpan) {
        if (position > self.leftVisibleWidth) {
            return 0.0f;
        } else if ((self.style == TXSidePanelMultipleActive || self.pushesSidePanels) && position < 0.0f) {
...
- (void)_loadCenterPanelWithPreviousState:(TXSidePanelState)previousState {
    [self _placeButtonForLeftPanel];
...
- (void)_showLeftPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    self.state = TXSidePanelLeftVisible;
    [self _loadLeftPanel];
    ...
    if (self.style == TXSidePanelSingleActive) {
...
- (void)_showRightPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    self.state = TXSidePanelRightVisible;
    [self _loadRightPanel];
...
- (void)toggleLeftPanel:(__unused id)sender {
    if (self.state == TXSidePanelLeftVisible) {
        [self _showCenterPanel:YES bounce:NO];
    } else if (self.state == TXSidePanelCenterVisible) {
        [self _showLeftPanel:YES bounce:NO];
    }
}
...
- (void)setCenterPanelHidden:(BOOL)centerPanelHidden animated:(BOOL)animated duration:(NSTimeInterval) duration {
    if (centerPanelHidden != _centerPanelHidden && self.state != TXSidePanelCenterVisible) {
