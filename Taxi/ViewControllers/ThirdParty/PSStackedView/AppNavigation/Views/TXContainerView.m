...
@implementation TXContainerView
...
+ (TXContainerView *)containerViewWithController:(UIViewController *)controller; {
    TXContainerView *view = [[TXContainerView alloc] initWithFrame:controller.view.frame];
...
- (void)setShadow:(PSSVSide)shadow {
    ...
    if (shadow) {
        if (!self.innerShadowLayer) {
            ...
            innerShadow.colors = [NSArray arrayWithObjects:(__bridge id)[UIColor colorWithWhite:0.0f alpha:shadowAlpha_].CGColor,
                                                           (__bridge id)[UIColor colorWithWhite:0.0f alpha:shadowAlpha_].CGColor, nil];
