   ...
   inAccessGroup: (NSString *)accessGroup
{
	...
	else
	{
		...
		if (FDIsEmpty(itemFromKeychain) == YES)
		{
			...
			if (result != noErr)
			{
				NSLog(@"Could not add keychain item. (Error code: %ld)",result);
			}
		}
		...
		else
		{
			...
			if (result != noErr)
			{
				NSLog(@"Could not update keychain item. (Error code: %ld)", result);
			}
		}
	}
}
           ...
           inAccessGroup: (NSString *)accessGroup
{
	...
	if (result != noErr)
	{
		...
		{
			NSLog(@"Could not delete keychain item. (Error code: %ld)",result);
		}
	}
}
